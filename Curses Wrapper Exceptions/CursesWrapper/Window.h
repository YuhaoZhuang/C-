...
class Window {
 ...
 protected:
  //throw an std::out_of_range exception if
  //row,col is not in bounds
  virtual void verifyInBounds(int row, int col) const;

  //return true if row,col is inside the window
  virtual bool isInBounds(int row, int col) const;
  //is val between low and high inclusive? low <= val <= high?
  virtual bool isBetween(int val, int low, int high) const;
